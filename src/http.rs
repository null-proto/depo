...
pub trait AsH1Compliance<'a> {
  fn h1_compliance_str(&'a mut self) -> &'a str;
}

impl<'a> AsH1Compliance<'a> for &'a mut str {
  fn h1_compliance_str(&'a mut self) -> &'a str {
    let mut is_first = true;

    unsafe {
      for i in self.as_bytes_mut() {
        if is_first {
  ...
  T: AsRef<[u8]>,
{
  fn into_h1_bytes(self) -> Vec<u8> {
    ...
    _res.extend_from_slice(status.as_str().as_bytes());
